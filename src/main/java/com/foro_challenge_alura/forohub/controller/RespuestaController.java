...
import com.foro_challenge_alura.forohub.domain.respuesta.validations.uptdate.ValidarRespuestaActualizada;
...
public class RespuestaController {
    ...
    @PostMapping
    @Transactional
    @Operation(summary = "Registra una nueva respuesta en la base de datos , vinculada a un usuario")
    ...
    public ResponseEntity<Page<DetalleRespuestaDTO>> leerRespuestaDeTopico(
            @PageableDefault(size = 5, sort = {"ultima_actualizacion"}, direction = Sort.Direction.ASC) Pageable pageable,
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Lee una unica respuesta por su Id")
    ...
    @PutMapping("/{id}")
    @Transactional
    @Operation(summary = "Actualiza el mensaje de la respuesta , la solicion o el estado de la respuesta")
